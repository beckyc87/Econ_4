...
```{r}
# cost to each sector
cost_A_tax <- integrate(demand_A_fxn, Q_A_tax, Q_A_initial)
...
total_cost_A_tax # 5403.34 total cost to sector A under tax (with addition of tax)

cost_B_tax <- integrate(demand_B_fxn, Q_B_tax, Q_B_initial)
total_cost_B_tax <- cost_B_tax[["value"]] + (Q_B_tax*tax)
total_cost_B_tax #5619.932

cost_C_tax <- integrate(demand_C_fxn, Q_C_tax, Q_C_initial)
total_cost_C_tax <- cost_C_tax[["value"]] + (Q_C_tax*tax)
total_cost_C_tax #11053.54
...
total_cost_tax #11519.4

cost_A_tax[["value"]] + cost_B_tax[["value"]] + cost_C_tax[["value"]] #11519.40
...
cost_A_trade <- integrate(demand_A_fxn, Q_A_tax, Q_A_initial)
cost_A_trade #2470.671 

cost_B_trade <- integrate(demand_B_fxn, Q_B_tax, Q_B_initial)
cost_B_trade #288.5947

cost_C_trade <- integrate(demand_C_fxn, Q_C_tax, Q_C_initial)
cost_C_trade #8760.131

total_cost_trade <- cost_A_trade[["value"]] + cost_B_trade[["value"]] + cost_C_trade[["value"]]
total_cost_trade #11519.40
...
#total emission Country X and Y is 600, or should we use 300

credit_all <- MC_agg_allsec_fxn(600)
credit_all #55.03763 cost carbon credit

#carbon emitted by each sector both countries
Q_A_trade <- A_MC_P(credit_all)
Q_B_trade <- B_MC_P(credit_all)
Q_C_trade <- C_MC_P(credit_all)
Q_D_trade <- D_MC_P(credit_all)

Q_A_trade #110.3979 tons carbon
Q_B_trade #230.044 tons carbon
Q_C_trade #85.08599 tons carbon
Q_D_trade #174.4722 tons carbon

Q_total_trade <- Q_A_trade + Q_B_trade + Q_C_trade + Q_D_trade
Q_total_trade #600 tons carbon
...
cost_A_trade_all <- integrate(demand_A_fxn, Q_A_trade, Q_A_initial)
cost_A_trade_all 

cost_B_trade_all <- integrate(demand_B_fxn, Q_B_trade, Q_B_initial)
cost_B_trade_all 

cost_C_trade_all <- integrate(demand_C_fxn, Q_C_trade, Q_C_initial)
cost_C_trade_all 

cost_D_trade_all <- integrate(demand_D_fxn, Q_D_trade, Q_D_initial)
cost_D_trade_all 

total_cost_trade_all <- cost_A_trade[["value"]] + cost_B_trade[["value"]] + cost_C_trade[["value"]] + cost_C_trade[["value"]]
...
country_x_pollution_2

country_y_pollution_2 <-Q_D_trade
country_y_pollution_2
