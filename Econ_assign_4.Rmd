...
```{r}
#Graph sector A
A_graph <- ggplot(carbon_s_d) +
  geom_smooth(aes(x = Abatement, y = MC_A), method = lm, se = FALSE, size = 0.5, color = "gray20") +
  geom_smooth(aes(x = Abatement, y = demandA), method = lm, se = FALSE, size = 0.5, color = "gray20") +
  theme_classic()+
  labs(title = "Sector A", 
...
#Graph sector B
B_graph <- ggplot(carbon_s_d) +
  geom_smooth(aes(x = Abatement, y = MC_B), method = lm, se = FALSE, size = 0.5, color = "gray20") +
  geom_smooth(aes(x = Abatement, y = demandB), method = lm, se = FALSE, size = 0.5, color = "gray20") +
  theme_classic()+
  labs(title = "Sector B", 
       x = "Carbon Emissions (Tons)",
       y = "Price ($/ton)") +
...
C_graph <- ggplot(carbon_s_d) +
  geom_smooth(aes(x = Abatement, y = MC_C), method = lm, se = FALSE, size = 0.5, color = "gray20") +
  geom_smooth(aes(x = Abatement, y = demandC), method = lm, se = FALSE, size = 0.5, color = "gray20") +
  theme_classic()+
  labs(title = "Sector C", 
       x = "Carbon Emissions (Tons)",
       y = "Price ($/ton)") +
...
D_graph <- ggplot(carbon_s_d) +
  geom_smooth(aes(x = Abatement, y = MC_D), method = lm, se = FALSE, size = 0.5, color = "gray20") +
  geom_smooth(aes(x = Abatement, y = demandD), method = lm, se = FALSE, size = 0.5, color = "gray20") +
  theme_classic()+
  labs(title = "Sector D", 
       x = "Carbon Emissions (Tons)",
       y = "Price ($/ton)") +
...
A_demand <- function(x) {(A_slope)*x + A_demand_int
}

B_demand <- function(x) {(B_slope)*x + B_demand_int
}

C_demand <- function(x) {(C_slope)*x + C_demand_int
}

D_demand <- function(x) {(D_slope)*x + D_demand_int
}
...
```{r}
#Sector cost
capcost_a <- integrate(A_demand, 80, A)
capcost_a #2644.085

capcost_b <- integrate(A_demand, 100, B)
capcost_b #1586.451

capcost_c <- integrate(A_demand, 120, C)
capcost_c #528.817 

#Total cost
capcost <- 2644.085 + 1586.451 + 528.817
capcost #Total cost of the carbon cap policy is $4759.35
...
A_supply <- function(x) {(model_A$coefficients[2])*x + model_A$coefficients[1]}

B_supply <- function(x) {(model_B$coefficients[2])*x + model_B$coefficients[1]}

C_supply <- function(x) {(model_C$coefficients[2])*x + model_C$coefficients[1]}

D_supply <- function(x) {(model_D$coefficients[2])*x + model_D$coefficients[1]}


#supply curve function solved for x, gives the quantity
A_supply_x <- function(y) {(y - model_A$coefficients[1]) / model_A$coefficients[2]}

B_supply_x <- function(y) {(y - model_B$coefficients[1]) / model_B$coefficients[2]}

C_supply_x <- function(y) {(y - model_C$coefficients[1]) / model_C$coefficients[2]}

D_supply_x <- function(y) {(y - model_D$coefficients[1]) / model_D$coefficients[2]}
...
A_slope_mc <- model_A$coefficients[2]
A_int_mc <- model_A$coefficients[1]
B_slope_mc <- model_B$coefficients[2]
B_int_mc <- model_B$coefficients[1]
C_slope_mc <- model_C$coefficients[2]
C_int_mc <- model_C$coefficients[1]
...
agg_mc_price <- function(q) {(q + (A_int_mc/A_slope_mc) + (B_int_mc/B_slope_mc) + (C_int_mc/C_slope_mc)) / ((1/A_slope_mc) + (1/B_slope_mc) + (1/C_slope_mc))}
tax_2 <- agg_mc_price(300)
tax_2

#carbon per sector with tax, currently this does not add up to 300 currently 
A_supply_x(tax_2)
B_supply_x(tax_2)
C_supply_x(tax_2)
